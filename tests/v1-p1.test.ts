...
import { fakeAcpProfile, replayProfile, requireDeepSeekWorkerLaunch, resolveClaudeLaunch } from "../src/workers/profiles.ts";
...
test("does not override worker model/provider/effort flags", () => {
  ...
  try {
    const deepseek = requireDeepSeekWorkerLaunch(repoRoot);
