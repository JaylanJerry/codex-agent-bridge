...
- `src/workers/deepseek-launch.ts` 成为唯一 DeepSeek launch 替换点。`resolveOfficialDeepSeekAcpRuntime()` 在 Gate 通过前保持 `undefined`。
- `doctor` / `bridge_agents` 报告 `transport=acp` 和 `integration=official|legacy|missing`，不再把“找到源码树”说成官方就绪。
- 文档不再承诺 DeepSeek 会继承用户当前持久 route。
- Core、Git 安全、Review、Verification 和 MCP public schema 保持稳定。
- 等 DeepSeek 提供正式可 spawn 的 ACP profile/runtime 后，另开实现 PR，不回头扩张 legacy 路径。
