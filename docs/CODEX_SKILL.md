...
Phase 2 诊断：`bridge_doctor`、`bridge_agents`、`bridge_status` 的 `needsAttention`。遗留 worktree 用 `bridge_prune`。MCP 默认 permission gate：`WAITING_FOR_INPUT` 时用 `bridge_respond`。`timeoutMs` 到期为 `TASK_TIMED_OUT`（可 continue）。改完 MCP 工具列表后需要新开 Codex 会话。
