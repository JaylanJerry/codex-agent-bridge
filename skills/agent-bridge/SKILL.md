...
`worker`：调试用 `replay`；闸门自检可用 `fake`；真干活用 `claude` 或 `deepseek`。V1 **只选 worker**。不要传、不要编造 `model` / `provider` / `effort`。Claude Code 沿用其原生配置。DeepSeek 当前是 legacy source ACP，effective route 由上游 ACP composition 决定，不是自动继承用户在 Web/TUI 里的当前持久选择。Claude 背后的官方 Anthropic、CC Switch 或其他第三方 Provider 是 Worker 内部实现，不要猜测 effective upstream model。
