...
type Native = {
  createKillOnCloseJob(): JobHandle;
  assignPidToJob(job: JobHandle, pid: number): void;
};

let native: Native | undefined;

function win32(): Native {
  if (process.platform !== "win32") {
    throw new Error("Job Object is only available on Windows");
  }
  native ??= bindWin32();
  return native;
}

function bindWin32(): Native {
  ...
  return {
    createKillOnCloseJob() {
      ...
      const limitOffset =
        koffi.offsetof(ExtendedLimitInformation, "BasicLimitInformation") +
        koffi.offsetof(BasicLimitInformation, "LimitFlags");
      info.writeUInt32LE(JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE, limitOffset);

      const ok = SetInformationJobObject(job, JobObjectExtendedLimitInformation, info, info.length);
      if (!ok) {
...
export function createKillOnCloseJob(): JobHandle {
  if (process.platform !== "win32") {
    return { ptr: null, close() {} };
  }
  return win32().createKillOnCloseJob();
}

export function assignPidToJob(job: JobHandle, pid: number): void {
  if (process.platform !== "win32") return;
  win32().assignPidToJob(job, pid);
}
