...
`bridge_run` **只选 worker**：`claude` 或 `deepseek`（调试可用 `replay` / `fake`）。不要传 `model` / `provider` / `effort`。Agent Bridge 不向任务接口暴露这些参数，也不会由 Codex 按 Task 选择。Claude Code 沿用其原生配置。DeepSeek 的 effective route 由所启动的 Harness ACP composition 决定；在官方 runtime 迁移完成前，legacy source 路径仍遵循上游 ACP composition 的配置，**不是**自动继承用户在 Web/TUI 里的当前持久选择。
